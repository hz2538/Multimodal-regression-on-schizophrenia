...
def sort_df(pred, true):
    ...
    df = pd.DataFrame(tmp,columns=['Predict','True'])
    return df
...
def lassoCD(data, label, ll, ul, step, state):
    ...
    print('selected features:',feature[np.where(r2==max(r2))][0])
    return pred, true, r2, mse, feature

def lassoLARS(data, label, ll, ul, step, state):
    kf = KFold(n_splits=10,shuffle=True,random_state=state)
    X = data
    y = label
    r2=[]
...
def lassoLARSIC(data, label, cri, state):
    ...
    r2=np.array(r2)
    mse = np.array(mse)
    f = feature[np.where(r2==max(r2))][0]
...
def reg_tree(data, label, lower_n_features, upper_n_features, state):
    ...
    print('maximum selected features:',np.argsort(np.mean(mat,axis=0))[:max(ilist)])
    return pred, true, r2, mse, mat
